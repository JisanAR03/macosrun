...
def check_for_changes(url, filename):
    new_content = fetch_webpage(url)
    old_content = load_content(filename)
    if old_content is not None:
        if has_content_changed(new_content, old_content):
            differences = get_differences(new_content, old_content)
            save_content(new_content, filename)  # Save the content immediately
            show_changed_content(new_content, differences)  # Display the changed content
        else:
            show_no_change_message()
    else:
        save_content(new_content, filename)  # Save the content for the first time
...
def show_changed_content(content, differences):
    window = tk.Toplevel()
...
def run_gui():
    ...
    submit_button = tk.Button(window, text="Monitor Changes", command=lambda: check_for_changes(url_entry.get(), filename_entry.get()))
    submit_button.pack()
